...
pub fn load_friend_list() -> HashMap<u32, [u8; 224]> {
    let mut friend_map = HashMap::new();

    unsafe {
        let mut frd_handle: Handle = 0;
        _ = ctru_sys::srvGetServiceHandle(&mut frd_handle as *mut _, c"frd:u".as_ptr());
        let (friend_keys, num_friends) = get_friend_keys(frd_handle);
        let friends = get_friend_info(frd_handle, &friend_keys, num_friends);
        _ = ctru_sys::svcCloseHandle(frd_handle);

        for i in 0..num_friends {
            let pid: u32 = friend_keys[i as usize].principalId;
            let mii_bytes: [u8; 0x5C] = friends[i as usize]._bindgen_opaque_blob[132..224]
                .try_into()
                .unwrap(); // Safe: known size
            friend_map.insert(pid, friends[i as usize]._bindgen_opaque_blob.clone());
        }
    }
...
unsafe fn get_friend_keys(handle: Handle) -> ([FriendKey; 100], u32) {
    let mut friend_keys: [FriendKey; 100] = [Default::default(); 100];
    ...
    unsafe {
        ...
        *cmdbuf.wrapping_add(2) = 100;
        // max number of friends is 100
        *cmdbuf.wrapping_add(64) = (FRIEND_LIST_SIZE << 18) | 2;
        *cmdbuf.wrapping_add(65) = &mut friend_keys as *mut _ as u32;
        ...
        (friend_keys, num_friends)
    }
}

unsafe fn get_friend_info(
    handle: Handle,
    friends: &[FriendKey; 100],
    num_friends: u32,
) -> [FriendInfo; 100] {
    unsafe {
        ...
        *cmdbuf.wrapping_add(5) = &friends as *const _ as u32;
        *cmdbuf.wrapping_add(6) =
            IPC_Desc_Buffer(num_friends * mem::size_of::<FriendInfo>() as u32);
        *cmdbuf.wrapping_add(7) = &mut friend_info as *mut _ as u32;
